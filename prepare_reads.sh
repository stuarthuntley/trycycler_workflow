...
if [ $# -eq 1 ]; then
    ...
    if ! [[ -f "$1" ]]; then
        echo "The input read file does not exist."
        exit 1
    fi
else
...
> reads.fq" > logs/filtlong.log

script -a logs/filtlong.log -c "filtlong --min_length 1000\
--keep_percent 95 '$read_file' > reads.fq"
